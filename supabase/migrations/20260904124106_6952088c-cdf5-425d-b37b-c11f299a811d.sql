-- تطبيقات التكامل الجديدة لكل مساحات العمل الحالية
insert into public.integrations (workspace_id, employee_id, provider, status)
select w.id, v.employee_id, v.provider, 'disconnected'
from public.workspaces w
cross join (values
  ('sonny','reddit'), ('sonny','bluesky'),
  ('eva','discord'), ('eva','zoom'), ('eva','trello'), ('eva','asana'),
  ('eva','jira'), ('eva','clickup'), ('eva','monday'), ('eva','telegram'),
  ('sam','pipedrive'), ('sam','intercom'), ('sam','twilio'),
  ('sam','airtable'), ('sam','stripe')
) as v(employee_id, provider)
where not exists (
  select 1 from public.integrations i
  where i.workspace_id = w.id and i.provider = v.provider
);

-- ونفس التطبيقات لأي مساحة عمل جديدة
...
BEGIN
  ...
  INSERT INTO public.integrations (workspace_id, employee_id, provider, status) VALUES
    ...
    (ws_id,'sonny','threads','disconnected'),

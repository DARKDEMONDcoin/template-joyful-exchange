-- تكاملات إضافية لكل موظف (نفس منطق منصات Marblism وأوسع)
INSERT INTO public.integrations (workspace_id, employee_id, provider, status)
SELECT w.id, x.employee_id, x.provider, 'disconnected'
FROM public.workspaces w
CROSS JOIN (VALUES
  ('sonny','youtube'),('sonny','threads'),('sonny','pinterest'),('sonny','google-business'),
  ('eva','outlook'),('eva','slack'),('eva','notion'),
  ('sam','salesforce'),('sam','mailchimp'),
  ('dana','drive'),
  ('adam','google-ads')
) AS x(employee_id, provider)
WHERE NOT EXISTS (
  SELECT 1 FROM public.integrations i
  WHERE i.workspace_id = w.id AND i.provider = x.provider
);
